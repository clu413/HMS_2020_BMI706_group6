---
title: "COVID tracking project - school closure"
author: "Jonathan Levin, Chenyue Lu, Kathleen Sucipto, Dany Thorpe"
date: "4/26/2020"
...
url_path <- 'https://covidtracking.com/api/v1/states/daily.csv'
download.file(url_path, destfile='covid_tracking_history.csv')
dat <- read_csv("covid_tracking_history.csv")
# convert date into factor
dat$date <- as.factor(dat$date)
school_closure_data <- read_csv("/Users/chenyuelu/Desktop/things/MBI/semester2/BMI706/group6/coronavirus-school-closures-data-4-7-2020.csv")
...
## add column "closed" to dat based on whether it is closed that day
school_closure_data$StateClosureStartDate %in% unique(dat$date)
unique(school_closure_data$StateClosureStartDate)
school_closure_data$StateClosureStartDate < unique(dat$date)
unique(dat$date)
...
dat <- dat %>%
  mutate(closed = date >= StateClosureStartDate) %>%
  arrange(desc(date))

dat[c('state','date','positive','negative','fips','StateClosureStartDate','closed')]
